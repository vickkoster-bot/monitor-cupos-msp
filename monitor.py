...
def extract_slots_from_page(
    ...
    matches = [normalize(match.group(0)) for match in SLOT_DATE_RE.finditer(body_text)]

    return [
        Slot(region=region, office=office, when=when, url=url)
        for when in dict.fromkeys(matches)
    ]
...
def monitor_region(page: Page, region: str, url: str) -> list[Slot]:
    ...
    if not controls:
        return extract_slots_from_page(
            page,
            region,
            "Vacunatorio mostrado",
            url,
        )
...
def format_summary(slots: Iterable[Slot]) -> str:
    ...
    for region in ("Sur", "Este", "Oeste", "Norte"):
        ...
        if count:
            ...
            lines.append(f"📍 {region}: {count} horario{suffix}")
        else:
...
def main() -> int:
        ...
        for region, url in REGIONS.items():
            try:
                ...
                try:
                    page.screenshot(
                        path=str(
                            SCREENSHOT_DIR / f"error-{region.lower()}.png"
                        ),
                        full_page=True,
                    )
                except Exception:
    ...
    for slot in all_slots:
        seen[slot.key] = {
            ...
            "when": slot.when,
            "url": slot.url,
            "first_seen_utc": seen.get(slot.key, {}).get(
                "first_seen_utc",
                now,
            ),
            "last_seen_utc": now,
    ...
    try:
        fingerprint, _page_text = official_page_fingerprint()
        ...
        logging.exception(
            "No se pudo revisar la página informativa oficial."
        )
    ...
    if new_slots:
        ...
        for region in ("Sur", "Este", "Oeste", "Norte"):
            ...
            if region_slots:
                send_telegram(
                    format_region_alert(region, region_slots)
                )
    else:
